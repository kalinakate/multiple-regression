...
Задачей пошаговой регрессии является упорядочивание независимых переменных X1, X2, X3 и т.д.???????? по важности их для предсказания ???? . Это означает,что необходимо найти такое подмножество из независимых переменных, которому бы соответствовал максимальный множественный коэффициент корреляции.@Конспект
